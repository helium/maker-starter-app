...
export const getPlaceGeography = async (placeId: string): Promise<PlaceGeography> => {
...
export const getCities = async (searchTerm: string): Promise<PlacePrediction[]> => {
  const response = await makeRequest("autocomplete", {
    input: searchTerm,
    type: "(cities)",
  });
  if ("predictions" in response) {
    return response.predictions.map((p: { description: string; place_id: string }) => ({
