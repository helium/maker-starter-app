...
export const getWalletAddress = async () => {
  return getItemAsync(walletAddressKey);
};

export const setWalletAddress = async (address: string) => {
  await setItemAsync(walletAddressKey, address);
};

export const getWalletToken = async () => {
  return getItemAsync(walletTokenKey);
};

export const setWalletToken = async (token: string) => {
  await setItemAsync(walletTokenKey, token);
};
// !! WALLET INFO KEYS
